...
numericToFactor <- function(df, colname) {
  df[colname]<-lapply(df[colname], function(x) as.factor(as.character(x)))
  return(df)
}
...
if (length(args)==0) {
  ...
  if (QuickTest) {
    ...
    qt.testid <- sample(nrow(qt.df), qt.pick_number)
    qt.testdf <- qt.df[ qt.testid, ]
    ...
    qt.models <- list()
    for (j in 1:length(qt.featureList)) {
      # remove feature
      features <- c(1:6)
      features <- features[-j]
      for (i in 1:length(qt.targetList)) {
        qt.models[[i]] <- svm(qt.traindf[features],qt.traindf[qt.targetList[[i]]], type = "eps-regression")
        qt.train_result[[i]] <- predict(qt.models[[i]], qt.traindf[features])
        qt.test_result[[i]] <- predict(qt.models[[i]], qt.testdf[features])
        qt.results <- rbind( qt.results, data.frame( target = qt.targetList[[i]],
                                                     type = "train",
                                                     note = qt.col[[j]],
                                                     MHS = MHSim(qt.train_result[[i]], qt.traindf[[qt.targetList[[i]]]]), 
  ...
  if (Main) {
    ...
    #main.targetList <- c("PDR", "PBR", "A12", "A15", "Light", "total")
      ...
      for (j in 1:length(main.folds)) {
        ...
        for (k in 1:main.folds[j]) {
          ...
          for (l in 1:2) {
            ...
            current.train.x <- current.train[, main.featureList]
            current.train.y <- current.train[, main.targetList[[l]]]
            current.test <- main.trainTables[[i]][-(start:end), ]
            current.test.x <- current.test[, main.featureList]
            ...
            if( length(main.bestmodels[[i]][[j]][[l]]) == 0 ){
              best.pre[[l]] <- list(train = current.train.result,
                                    test = current.test.result)
        ...
        main.trainTables[[i]]["prePDR"] <- predict(main.bestmodels[[i]][[j]][[1]]$model,
                                                   main.trainTables[[i]][, main.featureList])
        main.trainTables[[i]]["prePBR"] <- predict(main.bestmodels[[i]][[j]][[2]]$model,
                                                   main.trainTables[[i]][, main.featureList])
        main.calibrationTables[[i]]["prePDR"] <- predict(main.bestmodels[[i]][[j]][[1]]$model,
                                                         main.calibrationTables[[i]][, main.featureList])
        main.calibrationTables[[i]]["prePBR"] <- predict(main.bestmodels[[i]][[j]][[2]]$model,
                                                         main.calibrationTables[[i]][, main.featureList])
        ...
        for (k in 1:main.folds[j]) {
          ...
          # Train different model for different targets
          for (l in 3:length(main.targetList)) {
            current.train <- main.trainTables[[i]][start:end, ]
            current.train.x <- current.train[, c(main.featureList, main.addFeature)]
            current.train.y <- current.train[, main.targetList[[l]]]
            current.test <- main.trainTables[[i]][-(start:end), ]
            current.test.x <- current.test[, c(main.featureList, main.addFeature)]
            current.test.y <- current.test[, main.targetList[[l]]]
        ...
        for (l in 1:length(main.targetList)) {
          ...
          calibartion.x <- calibartion[, c(main.featureList, main.addFeature)]
          calibartion.y <- calibartion[, main.targetList[[l]]]
          if ( l <= 2 ){
            calibration.result <- calibartion[paste("pre",main.targetList[[l]],sep = "")][[1]]
          }
          else{
